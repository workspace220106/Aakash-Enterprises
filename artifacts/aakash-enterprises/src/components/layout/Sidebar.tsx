    ...
    <div className="glass-panel w-full h-full flex flex-col pt-8 pb-6 px-4 bg-white/80 backdrop-blur-md lg:bg-transparent lg:backdrop-blur-none lg:border-none">
      ...
      <nav className="flex-1 space-y-2">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className="block relative" onClick={onItemClick}>
              <div className={cn(
                "flex items-center gap-4 px-4 py-3.5 rounded-xl font-medium transition-all duration-200 group z-10 relative cursor-pointer",
                isActive 
                  ? "text-primary" 
